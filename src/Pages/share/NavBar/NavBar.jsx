...
const NavBar = () => {
  ...
  useEffect(() => {
    const search = async () => {
      ...
      const res = await axios.get(`${import.meta.env.VITE_URL}/colleges/${key}`, { params: { key: key, limit: 1 } })
        ...
        <div className="navbar pt-6">
          ...
          <div className="navbar-end res-end">
...

            {
              ...
              </div> : <Link to='/login'><button className="md:btn btn-primary md:bg-sky-800 btn-sm bg-sky-800 md:text-white hover:text-sky-800 ">Login</button></Link>
            }
              ...
              <div className='search-result'>
                {
                  searchResult.map((college, i) => (
                    <div key={i} className="card w-[365px] md:w-96 lg:w-96 sm:w-96 glass mb-8">
